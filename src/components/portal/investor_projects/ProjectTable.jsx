  ...
  return (
    <div className="min-h-full from-slate-50 to-slate-100">
      <div className="mx-auto">
        {/* Filters - moved outside scrollable/padded area for dropdown visibility */}
        <div className="bg-white rounded-xl shadow-md overflow-visible">
          <div className="p-6 border-b border-gray-200 z-30 relative">
            <div className="flex flex-wrap gap-3 relative z-30">
              ...
              <div className="relative date-filter-dropdown">
                <div className="relative">
                  ...
                  {dateDropdownOpen && (
                    <div className="absolute left-0 top-full mt-2 w-80 bg-white border border-gray-200 rounded-md shadow-lg z-20 p-4" style={{ minWidth: '250px', maxWidth: '100vw', overflow: 'auto' }}>
          ...
          <div className="flex flex-wrap gap-6 p-6">
            {Array.isArray(projects) && projects.length > 0 ? (
              projects.map((data, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative w-full h-36 sm:h-44 md:h-40 lg:h-36 xl:h-25 overflow-hidden">
                      <img
                        src={project.project_image || "/images/general/solar-card.jpg"}
                        alt={project.project_name}
                        className="object-cover w-full h-full"
                      />
                      <span className={`absolute top-2 right-2 px-3 py-1 text-xs font-semibold rounded-full shadow`}>{project.status}</span>
                    </div>
                    {/* Card content */}
                    <div className="p-3 md:p-4 flex flex-col flex-1">
                      ...
                      <div className="text-sm text-gray-600 mb-2">Offtaker: <span className="font-medium">{project.offtaker?.fullName}</span></div>
                      {/* Stats boxes */}
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-3">
                        ...
                        <div className="bg-gray-50 rounded-lg p-2 text-center">
                          <div className="text-base font-bold text-amber-600">{project.project_size} kWh/year</div>
                          <div className="text-xs text-gray-500">Expected Generation</div>
                        </div>
                        <div className="bg-gray-50 rounded-lg p-2 text-center">
                          <div className="text-base font-bold text-orange-600">{project.expected_roi ? `${project.expected_roi}%` : "N/A"}</div>
                      ...
                      <div className="flex flex-col md:flex-row gap-2 bg-gray-100 rounded-lg p-2 mb-3 text-center text-xs font-medium text-gray-700">
                        <div className="flex-1 md:border-r border-gray-300">
                          ...
                          <div className="text-lg font-bold text-slate-900">{project.lease_term ? `${project.lease_term} years` : "N/A"}</div>
                        </div>
              ...
              <div className="text-gray-500 text-center w-full relative z-10">No projects found.</div>
            )}
          </div>
        </div>
      </div>
