...
function DashboardView() {
  ...
  useEffect(() => {
    const fetchInverters = async () => {
      ...
      try {
        const res = await apiGet(`/api/project-inverters?project_id=${selectedProject.id}`);
        if (res?.success && Array.isArray(res?.data)) {
          ...
          {showProjectsDropdown && (
            ...
            >
              ...
              >
                ...
                {projectsLoading ? (
                  <li style={{ padding: '8px 16px', color: '#6b7280' }}>Loading...</li>
                ) : projectsError ? (
                  <li style={{ padding: '8px 16px', color: '#b45309' }}>{projectsError}</li>
                ) : projects.length ? (
                  projects.map((proj) => {
                    const isSelected =
                      selectedProject &&
                      (selectedProject.id === proj.id || selectedProject.slug === proj.slug);
                    return (
                      <li
                  ...
                  <li style={{ padding: '8px 16px', color: '#6b7280' }}>No projects available.</li>
                )}
              </ul>
            </div>
          )}
            ...
            >
              ...
              >
                {invertersLoading ? (
                  ...
                  <li style={{ padding: '8px 16px', color: '#b45309' }}>{invertersError}</li>
                ) : inverters.length ? (
                  inverters.map((inv) => {
                    const isSelectedInv =
                      selectedInverter &&
