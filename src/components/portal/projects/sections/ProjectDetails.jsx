...
const ProjectDetails = ({ project_id }) => {
    ...
    const fetchExistingReview = async (projectId) => {
        ...
        try {
            ...
            const res = await apiGet(`/api/testimonials/user-review?project_id=${projectId}`);
    ...
    useEffect(() => {
        const fetchProjectById = async () => {
            ...
            try {
                const res = await apiGet(`/api/projects/${project_id}`);
                if (res?.success && res?.data) {
    ...
    useEffect(() => {
        if (selectedProject?.id) {
            fetchExistingReview(selectedProject.id);
        } else {
            setExistingReview(null);
        }
    }, [selectedProject?.id]);
    ...
    useEffect(() => {
        ...
        if (selectedProject && selectedMonthYear) {
    ...
    return (
        <div>
            <>
                ...
                >
                    ...
                    <button
                        ...
                        style={{
                            minWidth: "130px",
                            backgroundColor: existingReview ? "#10B981" : "#F6A623",
                            opacity: selectedProject && !reviewLoading ? 1 : 0.6,
                            cursor: selectedProject && !reviewLoading ? "pointer" : "not-allowed",
                        }}
                        disabled={!selectedProject || reviewLoading}
                    >
                        {reviewLoading
            ...
            <ReviewModal
                ...
                onReviewUpdated={() => fetchExistingReview(selectedProject?.id)}
            />
        </div>
    );
}
