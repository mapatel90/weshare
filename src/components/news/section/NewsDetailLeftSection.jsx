...
const NewsDetailLeftSection = ({ news, previousNews, nextNews }) => {
  ...
  return (
    <div className="col-md-8">
      <h2 className="article-title mb-3">{news.news_title}</h2>
      ...
      <div className="article-section">
        <div dangerouslySetInnerHTML={createMarkup(news.news_description)} />
      </div>

      <div className="d-flex justify-content-between align-items-center mt-4">
        {previousNews ? (
          <Link
            href={`/newsDetail/${previousNews.news_slug}`}
        ...
        {nextNews ? (
          <Link
            href={`/newsDetail/${nextNews.news_slug}`}
            className="text-decoration-none text-dark next-pre-btn"
          >
            {lang("news.next")}
